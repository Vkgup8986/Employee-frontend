...
const EditDepartment = () => {
      ...
      const fetchDepartments = async () => {
        ...
        try {
          const response = await axios.get(
           `https://employee-server-i9od.vercel.app/api/department/${id}`, {
            headers: {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    try {
      const response = await axios.put( `https://employee-server-i9od.vercel.app/api/department/${id}`, department, {
